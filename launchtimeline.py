...
def hard_cleanup():
    ...
    try:
        ...
        callback_ids = [
        "myTimelineProSaveCallback",       
        "myTimelineProSelectionCallback" 
    ]
        
        for cb_id in callback_ids:
            
            rt.callbacks.removeScripts(id=rt.name(cb_id))
    except Exception as e:
        print(f"   - Warning removing callbacks: {e}")
